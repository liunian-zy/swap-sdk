...
import { CurrencyAmount as CurrencyAmount4, Token as Token6 } from "@pancakeswap/swap-sdk-core";
...
var Fetcher = class {
  ...
  static async fetchTokenData(chainId, address, provider = getDefaultProvider(getNetwork(chainId)), symbol, name) {
    ...
    return new Token6(chainId, address, parsedDecimals, symbol, name);
  }
  /**
  ...
  static async fetchPairData(tokenA, tokenB, provider = getDefaultProvider(getNetwork(tokenA.chainId))) {
    invariant7(tokenA.chainId === tokenB.chainId, "CHAIN_ID");
    const address = Pair.getAddress(tokenA, tokenB);
    const [reserves0, reserves1] = await new Contract(address, IPancakePair_default, provider).getReserves();
    const balances = tokenA.sortsBefore(tokenB) ? [reserves0, reserves1] : [reserves1, reserves0];
    return new Pair(
      CurrencyAmount4.fromRawAmount(tokenA, balances[0]),
      CurrencyAmount4.fromRawAmount(tokenB, balances[1])
    );
  }
};
